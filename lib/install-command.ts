...
export function generateInstallCommand({ 
  ...
  // Clean up repository URL to extract meaningful parts
 ...
 */
function extractRepositoryInfo(cleanUrl: string): { owner: string; name: string } | null {
  // GitHub patterns
...
export function generateInstallCommandWithScopes({ 
  ...
  const baseCommand = generateInstallCommand({ repoUrl, authorHandle, slug, skillName });
 ...
 */
export function getSkillDisplayName({ repoUrl, authorHandle, slug, skillName }: InstallCommandOptions): string {
  const repoInfo = extractRepositoryInfo(repoUrl.replace(/^https?:\/\//, '').replace(/\.git$/, ''));
