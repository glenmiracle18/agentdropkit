...
export default function SkillDetailClient({ skill }: SkillDetailClientProps) {
  ...
  const [installScope, setInstallScope] = useState<
  ...
  return (
    <>
      ...
      <div className="text-xs sm:text-sm font-bold text-text-muted mb-6 md:mb-10 flex flex-wrap items-center gap-2 sm:gap-3 uppercase tracking-widest">
        ...
        <span>/</span>
        <span className="text-text-primary truncate max-w-[100px] sm:max-w-none">
          {skill.authorHandle}
        </span>
        <span>/</span>
      ...
      <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
        ...
        <div className="flex-1 min-w-0">
          <div className="mb-8 md:mb-10 flex flex-col sm:flex-row sm:items-start justify-between gap-6">
            <div>
              <h1 className="text-2xl sm:text-4xl font-bold text-text-primary mb-1 sm:mb-2 truncate">
                {skill.name}
              </h1>
              <p className="text-text-muted text-base sm:text-lg truncate">
                {skill.authorHandle}
              </p>
            </div>
            ...
            >
              <svg
                className={`w-5 h-5 sm:w-6 sm:h-6 ${hasUpvoted ? "fill-white" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                strokeWidth="2"
          ...
          <div className="mb-8 md:mb-12">
            ...
            <div className="mt-3 text-xs text-text-dim">
              {installScope === "default" && (
                <p>
                  Default installation - CLI will prompt you to choose between
                  global and project scope
                </p>
              )}
              {installScope === "global" && (
                <p>
                  Global installation - Available across all projects in{" "}
                  <code className="bg-bg-card px-1 py-0.5 font-mono">
                    ~/.claude/skills/
                  </code>
                </p>
              )}
              {installScope === "project" && (
                <p>
                  Project installation - Only available in current project
                  directory{" "}
                  <code className="bg-bg-card px-1 py-0.5 font-mono">
                    ./claude/skills/
                  </code>
          ...
          {skill.files && skill.files.length > 0 && (
            <div className="mb-8 md:mb-12">
              ...
              <div className="flex flex-col md:flex-row border-2 border-border bg-bg-card shadow-[4px_4px_0px_0px_var(--color-border)] md:shadow-[6px_6px_0px_0px_var(--color-border)]">
                ...
                {/* File Content */}
                <div className="flex-1 overflow-x-auto bg-bg-card min-h-[300px] md:max-h-[550px] relative flex flex-col">
                  {selectedFile ? (
                    <>
                      <div
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6 md:space-y-8">
          ...
          <div className="bg-bg-card border-2 border-border p-6">
            <h3 className="text-sm font-bold text-text-muted uppercase tracking-widest mb-6 border-b-2 border-border pb-2">
              Details
            </h3>
            ...
            <div className="space-y-6">
              <div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-accent hover:underline font-bold text-sm break-all"
                >
                  {skill.repository}
                </a>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skill.tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="text-xs font-bold uppercase tracking-widest border-2 border-border bg-bg-base px-2 py-1 text-text-primary"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div>
                ...
                <div className="flex items-end gap-2">
                  <span className="text-4xl font-bold text-success leading-none">
                    {skill.healthScore >= 95
                      ? "A+"
                      : skill.healthScore >= 90
                        ? "A"
                        : "B+"}
                  </span>
                  <span className="text-lg font-bold text-text-muted leading-none mb-0.5">
